...
/* Below, write_receive_bit and read_receive_bit are implemented as (very)
 * simple coroutines that use C preprocessor liberally, namely: a custom domain
 * specific language for yielding and waiting for bits.
 *
 * A coroutine remember its state in the global state variable (i.e. its line
 * number, as a rudimentary subsitute for program counter), and resume execution
 * when called again.
 */
